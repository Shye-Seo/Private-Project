$(function () {
        ...
        function submitDate(actionUrl){
            if (confirm("등록하시겠습니까?") == true){
                ...
                for(let i = 0; i < deleteFileNameList.length; i++){
                    $('form[name="partnerAddForm"]').append("<input type='hidden' name='deleteFileNameList' value='"+deleteFileNameList[i]+"'/>")
                }
                for(let i = 0; i < deleteFileNameList_thumbnail.length; i++){
                    $('form[name="partnerAddForm"]').append("<input type='hidden' name='deleteFileNameList_thumbnail' value='"+deleteFileNameList_thumbnail[i]+"'/>")
                }

                // var uploadFileList = Object.keys(fileList);
                var form = $('form[name="partnerAddForm"]');
                var formData = new FormData(form[0]);
                for (var i = 0; i < fileList.length; i++) {
                    formData.append('partner_file', fileList[i]);
                    console.log(formData.get('partner_file')) 
   ...
   function addFileList_thumbnail(fIndex_thumbnail, fileName_thumbnail, fileSizeStr_thumbnail) {
       ...
       $('input[name=partner_poster]').val(fileName_thumbnail);
   }
