        ...
        function submitDate(actionUrl){
            if (confirm("등록 하시겠습니까?") == true){
                ...
                for(let i = 0; i < deleteFileNameList.length; i++){
                    $('form[name="noticeAddForm"]').append("<input type='hidden' name='deleteFileNameList' value='"+deleteFileNameList[i]+"'/>")
                }

                // var uploadFileList = Object.keys(fileList);
                var form = $('form[name="noticeAddForm"]');
                var formData = new FormData(form[0]);
                for (var i = 0; i < fileList.length; i++) {
                    formData.append('notice_file', fileList[i]);
                    console.log(formData.get('notice_file')) 
