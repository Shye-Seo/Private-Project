$(function () {
        ...
        function submitDate(actionUrl){
            if (confirm("등록 하시겠습니까?") == true){
                ...
                for(let i = 0; i < deleteFileNameList_thumbnail.length; i++){
                    $('form[name="eduAddForm"]').append("<input type='hidden' name='deleteFileNameList_thumbnail' value='"+deleteFileNameList_thumbnail[i]+"'/>")
                }

                // var uploadFileList = Object.keys(fileList);
                var form = $('form[name="eduAddForm"]');
                var formData = new FormData(form[0]);
                for (var i = 0; i < fileList.length; i++) {
                    formData.append('edu_file', fileList[i]);
                    console.log(formData.get('edu_file')) 
   ...
   function addFileList_thumbnail(fIndex_thumbnail, fileName_thumbnail, fileSizeStr_thumbnail) {
       ...
       $('input[name=edu_poster]').val(fileName_thumbnail);
   }
